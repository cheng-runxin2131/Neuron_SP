...
class DeepSpeedEngine(Module):
    ...
    def __init__(self,
        ...
        # M317 (Claude-22): Megatron-style bucket AllReduce scheduler
        # Ref: Megatron-LM/megatron/core/distributed/param_and_grad_buffer.py
        #   _ParamAndGradBucket — groups params into contiguous grad buffers,
        #   then runs AllReduce per-bucket with async overlap.
        # DES-LOC extends this: each bucket's AllReduce is gated by Kx.
        # On non-Kx steps, gradients accumulate locally (no comm).
        # On Kx boundary steps, all pending buckets are reduced at once.
        self._desloc_buckets = []          # list of DeslocGradBucket
        self._desloc_bucket_size = 25_000_000  # ~100MB per bucket (fp32)
        self._desloc_buckets_initialized = False
        self._desloc_async_handles = []    # pending NCCL handles
        self._desloc_comm_bytes_total = 0  # cumulative bytes communicated
        self._desloc_comm_ops_total = 0    # cumulative AllReduce ops
        # Per-tier comm tracking (Ref: NCCL src/collectives.cc ncclAllReduce)
        self._desloc_tier_stats = {
            'x': {'count': 0, 'bytes': 0, 'time_ms': 0.0},  # params
            'u': {'count': 0, 'bytes': 0, 'time_ms': 0.0},  # 1st moment
            'v': {'count': 0, 'bytes': 0, 'time_ms': 0.0},  # 2nd moment
        }
        # NKI-FA style structured log buffer — parsed by draw_plot.py
        self._desloc_nkifa_log = []   # list of log line strings
    ...
    # ─── M317 (Claude-22): Megatron-style Bucket AllReduce for DES-LOC ───
    # Ref: Megatron-LM megatron/core/distributed/param_and_grad_buffer.py
    #   class _ParamAndGradBucket — groups params into contiguous buffers
    #   class _ParamAndGradBucketGroup — aggregates bucket comms
    # Ref: NCCL src/device/all_reduce.h — Ring/Tree AllReduce kernel
    # Ref: neuronx-distributed src/neuronx_distributed/optimizer/
    #   zero_redundancy_optimizer.py — NeuronZero1Optimizer (Trainium path)
    #
    # Design: DES-LOC extends Megatron's bucket concept with Kx-gated sync.
    #   On non-Kx steps: grads accumulate in local buckets (zero comm).
    #   On Kx boundary: all buckets run async AllReduce, then wait.
    #   Each bucket tracks its own comm stats for NKI-FA logging.
    #
    # From Nick Joseph: "真正起决定作用的是算力"
    #   → We maximize MFU by overlapping bucket AllReduce with next micro-batch.

    def _desloc_init_buckets(self):
        """Initialize gradient buckets for bucket-aware DES-LOC AllReduce.

        Mirrors Megatron _ParamAndGradBuffer.__init__:
          1. Sort params by decreasing numel (larger params = own bucket)
          2. Pack params into buckets up to bucket_size
          3. Allocate contiguous grad buffer per bucket

        Called lazily on first backward pass (after model params are known).
        """
        if self._desloc_buckets_initialized:
            return
        self._desloc_buckets_initialized = True
        ...
        # Sort by numel descending — Megatron convention for bucket packing
        ...
        for name, p in params:
            ...
            if (current_bucket_params and
                    current_bucket_numel + param_numel > self._desloc_bucket_size):
                self._desloc_create_bucket(bucket_id, current_bucket_params)
                bucket_id += 1
                current_bucket_params = []
                current_bucket_numel = 0

            current_bucket_params.append((name, p))
            current_bucket_numel += param_numel

        # Flush remaining params into last bucket
    ...
    def _desloc_create_bucket(self, bucket_id, named_params):
        ...
        Ref: Megatron _ParamAndGradBucket.__init__
          ...
          - grad_data: contiguous gradient buffer (view into flat tensor)
    ...
    def _desloc_bucket_allreduce(self):
        ...
        Ref: Megatron BucketGroup.finish_grad_sync — waits for all
          async AllReduce handles.
        Ref: NCCL ncclAllReduce (src/collectives.cc line 111)
          — Ring AllReduce for small messages, Tree for large.

        DES-LOC extension: only fires on Kx boundary steps.
        On non-Kx steps, this function is a no-op (zero comm).
        """
        import time as _t

        if not self._desloc_buckets:
            return

        # Fill buckets with current gradients
        self._desloc_fill_buckets()

        # Gate: only communicate on Kx boundary
        if not self.desloc_is_param_sync_step():
            self.desloc_skipped_allreduces += len(self._desloc_buckets)
            return

        # Async AllReduce on each bucket
        # Ref: Megatron uses torch.distributed.all_reduce with async_op=True
        handles = []
        ...
        elapsed_ms = (_t.perf_counter_ns() - t0) / 1e6
        for tier in self._desloc_tier_stats:
            if self._desloc_tier_stats[tier]['count'] > 0:
                self._desloc_tier_stats[tier]['time_ms'] += elapsed_ms

        # Scatter reduced gradients back to params
        self._desloc_scatter_buckets()
    ...
    def _desloc_momentum_sync(self):
        ...
        # Ref: neuronx-distributed ZeRO-1 pattern — iterate state_dict
        for group in self.optimizer.param_groups:
            for p in group['params']:
                if not p.requires_grad or p not in self.optimizer.state:
                    continue
                ...
                if sync_u and 'exp_avg' in state:
                    ...
                    self._desloc_tier_stats['u']['count'] += 1
                    self._desloc_tier_stats['u']['bytes'] += (
                ...
                if sync_v and 'exp_avg_sq' in state:
                    ...
                    self._desloc_tier_stats['v']['count'] += 1
                    self._desloc_tier_stats['v']['bytes'] += (
    ...
    def _desloc_log_nkifa(self, loss=None, lr=None):
        ...
        Ref: NKI-FA commit da964f3 draw_plot.py — parses ### config ### blocks
        Format: ### key=val, key=val ### then metric: value lines
        """
        if dist.get_rank() != 0:
            return
        step = self.desloc_step
        is_sync = self.desloc_is_param_sync_step()
        loss_str = f"{loss:.4f}" if loss is not None else "N/A"
        lr_str = f"{lr:.6f}" if lr is not None else "N/A"

        line = (f"step={step} loss={loss_str} lr={lr_str} "
                f"sync={'Y' if is_sync else 'N'} "
                f"Kx={self.desloc_Kx} Ku={self.desloc_Ku} Kv={self.desloc_Kv} "
                f"skipped={self.desloc_skipped_allreduces} "
                f"comm_bytes={self._desloc_comm_bytes_total} "
                f"buckets={len(self._desloc_buckets)}")
        self._desloc_nkifa_log.append(line)

        # Print every log_interval steps
        if step % max(1, getattr(self._config, 'steps_per_print', 100)) == 0:
            print(f"[DES-LOC] {line}")

    def _desloc_export_nkifa_log(self, output_dir):
        """Export NKI-FA format log file.

        Output: one ### block per experiment, metrics as key: value lines.
        """
        if dist.get_rank() != 0:
            return
        import os
        ...
        path = os.path.join(output_dir,
                            f'desloc_Kx{self.desloc_Kx}_Ku{self.desloc_Ku}_Kv{self.desloc_Kv}.log')
        with open(path, 'w') as f:
            f.write(f"### model = {getattr(self._config, 'model_size', 'unknown')}, "
                    f"method = DES-LOC, "
                    f"Kx = {self.desloc_Kx}, Ku = {self.desloc_Ku}, "
                    f"Kv = {self.desloc_Kv}, "
                    f"world_size = {dist.get_world_size()} ###\n")
            for line in self._desloc_nkifa_log:
                f.write(line + '\n')
            # Summary stats
            f.write(f"\ntotal_comm_bytes: {self._desloc_comm_bytes_total}\n")
            f.write(f"total_comm_ops: {self._desloc_comm_ops_total}\n")
            f.write(f"skipped_allreduces: {self.desloc_skipped_allreduces}\n")
            for tier, stats in self._desloc_tier_stats.items():
                f.write(f"tier_{tier}_count: {stats['count']}\n")
                f.write(f"tier_{tier}_bytes: {stats['bytes']}\n")
                f.write(f"tier_{tier}_time_ms: {stats['time_ms']:.2f}\n")

    def _desloc_get_comm_reduction(self):
        """Compute DES-LOC communication reduction ratio.

        Ref: DES-LOC paper Section 3 — Theorem 2
          reduction = K_DDP / (syncs_x + syncs_u + syncs_v)
          where K_DDP = total_steps (DDP syncs every step)
        """
        if self.desloc_step == 0:
            return 1.0
        ddp_syncs = self.desloc_step  # DDP would sync every step
        desloc_syncs = sum(s['count'] for s in self._desloc_tier_stats.values())
        if desloc_syncs == 0:
            return float('inf')
        return ddp_syncs / desloc_syncs
