...
"""GPT-2 Tokenization classes. Ported from Megatron-LM 46379244a (megatron/tokenizer/gpt2_tokenization.py)."""
...
_gpt2_logger = _logging.getLogger(__name__ + '.gpt2_tokenizer')

_GPT2_PRETRAINED_VOCAB_ARCHIVE_MAP = {
    'gpt2': "https://s3.amazonaws.com/models.huggingface.co/bert/gpt2-vocab.json",
}
_GPT2_PRETRAINED_MERGES_ARCHIVE_MAP = {
    'gpt2': "https://s3.amazonaws.com/models.huggingface.co/bert/gpt2-merges.txt",
}
_GPT2_PRETRAINED_VOCAB_POSITIONAL_EMBEDDINGS_SIZE_MAP = {
    'gpt2': 1024,
}
_GPT2_VOCAB_NAME = 'vocab.json'
_GPT2_MERGES_NAME = 'merges.txt'
_GPT2_SPECIAL_TOKENS_NAME = 'special_tokens.txt'
...
class GPT2Tokenizer(object):
    ...
    Ported from Megatron-LM commit 46379244a (megatron/tokenizer/gpt2_tokenization.py).
    """
    @classmethod
    def from_pretrained(cls, pretrained_model_name_or_path, cache_dir=None, *inputs, **kwargs):
        ...
        """
        if pretrained_model_name_or_path in _GPT2_PRETRAINED_VOCAB_ARCHIVE_MAP:
            vocab_file = _GPT2_PRETRAINED_VOCAB_ARCHIVE_MAP[pretrained_model_name_or_path]
            merges_file = _GPT2_PRETRAINED_MERGES_ARCHIVE_MAP[pretrained_model_name_or_path]
            special_tokens_file = None
        else:
            vocab_file = os.path.join(pretrained_model_name_or_path, _GPT2_VOCAB_NAME)
            merges_file = os.path.join(pretrained_model_name_or_path, _GPT2_MERGES_NAME)
            special_tokens_file = os.path.join(pretrained_model_name_or_path, _GPT2_SPECIAL_TOKENS_NAME)
            if not os.path.exists(special_tokens_file):
                special_tokens_file = None
            else:
                _gpt2_logger.info("loading special tokens file {}".format(special_tokens_file))
        try:
            ...
            _gpt2_logger.error(
                ...
                "at this path or url.".format(
                    ...
                    ', '.join(_GPT2_PRETRAINED_VOCAB_ARCHIVE_MAP.keys()),
                    pretrained_model_name_or_path,
                    vocab_file, merges_file))
            return None
        if resolved_vocab_file == vocab_file and resolved_merges_file == merges_file:
            _gpt2_logger.info("loading vocabulary file {}".format(vocab_file))
            _gpt2_logger.info("loading merges file {}".format(merges_file))
        else:
            _gpt2_logger.info("loading vocabulary file {} from cache at {}".format(
                vocab_file, resolved_vocab_file))
            _gpt2_logger.info("loading merges file {} from cache at {}".format(
                merges_file, resolved_merges_file))
        if pretrained_model_name_or_path in _GPT2_PRETRAINED_VOCAB_POSITIONAL_EMBEDDINGS_SIZE_MAP:
            max_len = _GPT2_PRETRAINED_VOCAB_POSITIONAL_EMBEDDINGS_SIZE_MAP[pretrained_model_name_or_path]
    ...
    def set_special_tokens(self, special_tokens):
        ...
        _gpt2_logger.info("Special tokens {}".format(self.special_tokens))

    def bpe(self, token):
        if token in self.cache:
            return self.cache[token]
        word = tuple(token)
        pairs = gpt2_get_pairs(word)
    ...
    def save_vocabulary(self, vocab_path):
        ...
        if not os.path.isdir(vocab_path):
            _gpt2_logger.error("Vocabulary path ({}) should be a directory".format(vocab_path))
            return
        vocab_file = os.path.join(vocab_path, _GPT2_VOCAB_NAME)
        merge_file = os.path.join(vocab_path, _GPT2_MERGES_NAME)
        special_tokens_file = os.path.join(vocab_path, _GPT2_SPECIAL_TOKENS_NAME)
        ...
        with open(merge_file, "w", encoding="utf-8") as writer:
            ...
            for bpe_tokens, token_index in sorted(self.bpe_ranks.items(), key=lambda kv: kv[1]):
                if index != token_index:
                    _gpt2_logger.warning("Saving vocabulary to {}: BPE merge indices are not consecutive."
        ...
        with open(special_tokens_file, 'w', encoding='utf-8') as writer:
            for token, token_index in sorted(self.special_tokens.items(), key=lambda kv: kv[1]):
                if index != token_index:
                    _gpt2_logger.warning("Saving special tokens vocabulary to {}: BPE indices are not consecutive."
