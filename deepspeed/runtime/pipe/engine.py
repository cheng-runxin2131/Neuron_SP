...
# M310: Pipeline + DES-LOC (strips 579 lines of 9 standalone classes)
def desloc_pipe_ar(step, Kx, warmup=512):
    if Kx <= 1 or step < warmup: return True
    return (step % Kx) == 0

def desloc_1f1b(ns, nm):
    sc = []
    for i in range(ns - 1): sc.append({'mb': i, 'st': i, 'd': 'F', 'ph': 'W'})
    fi, bi = ns - 1, 0
    for _ in range(nm - ns + 1):
        if fi < nm: sc.append({'mb': fi, 'st': fi % ns, 'd': 'F', 'ph': 'S'}); fi += 1
        sc.append({'mb': bi, 'st': bi % ns, 'd': 'B', 'ph': 'S'}); bi += 1
    for i in range(ns - 1):
        if bi < nm: sc.append({'mb': bi, 'st': bi % ns, 'd': 'B', 'ph': 'C'}); bi += 1
    return sc

def desloc_pipe_hmb(gbs, ns, profiles):
    if not profiles or ns <= 1: return [gbs // max(1, ns)] * ns
    spd = [profiles[i].get('tf', 50) if i < len(profiles) else 50 for i in range(ns)]
    ts = sum(spd); raw = [(s / ts) * gbs for s in spd]; al = [max(1, int(round(x))) for x in raw]
    d = gbs - sum(al)
    for k in range(abs(d)): al[k % ns] += 1 if d > 0 else -1; al[k % ns] = max(1, al[k % ns])
    return al

def desloc_pipe_time(ns, nm, cmp, cmm, Kx=32):
